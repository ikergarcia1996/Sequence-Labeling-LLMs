#!/bin/bash
#SBATCH --job-name=medT5-zero
#SBATCH --cpus-per-task=16
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --output=medT5-zero.out.txt
#SBATCH --error=medT5-zero.err.txt
...
do
  ...
  --train_tsvs /ikerlariak/igarcia945/antidote_mt5-corpus/sequence-labeling-evaluation-datasets/en/en-e3c-train.tsv \
  --dev_tsvs \
  ...
  --output_dir ./Antidote_mT5_zero/en/en-e3c/"$modelparams" \
  --seed 42 \
  ...
  --project_name "Antidote_mT5_zero" \
  --add_labels_as_tokens


  rm -rf ./Antidote_mT5_zero/en/en-e3c/"$modelparams"/*.bin
